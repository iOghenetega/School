...
from student_management.decorators import (
    ...
    staff_required, instructor_required
)
from student_management.School.serializer_course import course_retrieve_fields_serializer
from http import HTTPStatus


students_namespace = Namespace(
    'students', description='Namespace for Student ')


students_serializer = students_namespace.model(
    'Students list serializer', students_fields_serializer)
courses_serializer = students_namespace.model(
    'Students courses list serializer', course_retrieve_fields_serializer)
courses_add_serializer = students_namespace.model(
    'Courses add serializer', course_fields_serializer)
student_score_add_serializer = students_namespace.model(
    'Courses add serializer', student_score_add_fields_serializer)
        ...
        if student_in_course:
            ...
            if score:
                ...
                score = Score(student_id=student_id,
                              course_id=course_id, score=score_value, grade=grade)
            try:
                score.save()
...
class StudentGPAView(Resource):
    ...
    def get(self, student_id):
        ...
        for course in courses:
            ...
            score_exist = Score.query.filter_by(
                student_id=student.id, course_id=course.id).first()
            if score_exist:
