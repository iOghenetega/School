from student_management.models import (
    Student, Course,  Instructor,  StudentCourse, Score)
from student_management.decorators import (
    staff_required, admin_required, instructor_required, student_required)
...
course_creation_serializer = courses_namespace.model(
    'Course creation serializer', {
        ...
        'instructor_id': fields.Integer(required=True, description="Course teacher id"),
    }
)
...
class CoursesListView(Resource):
    ...
    @courses_namespace.doc(
        description="""
            ...
            It allows the retrieval of all available in the school
            """
    )
    @jwt_required()
    def get(self):
    ...
    @courses_namespace.doc(
        description="""
            ...
            It allows admins create new courses
            """
    )
    @admin_required()
    def post(self):
        ...
        if instructor:
            ...
            try:
                course.save()
                return {'message': 'You have registered this course successfully'}, HTTPStatus.CREATED
            except:
...
class CourseRetrievalView(Resource):
    ...
    @courses_namespace.doc(
        description="""
            This endpoint is accessible to only admins. 
            It allows admins delete a course
            """
    )
    @admin_required()
...
class CourseRetrievalView(Resource):
    ...
    @courses_namespace.doc(
        description="""
            This endpoint is accessible to only instructors. 
            It allows instructors add a  student the their course
            """
    )
    @instructor_required()
    def post(self, course_id):
        ...
        if student:
            ...
            get_student_in_course = StudentCourse.query.filter_by(
    ...
    @courses_namespace.doc(
        description="""
            This endpoint is accessible to only instructors. 
            It allows instructors remove a  student from their course
            """
    )
    @instructor_required()
    def delete(self, course_id):
        ...
        if student:
            ...
            if get_student_in_course:
                try:
                    ...
                    db.session.rollback()
                    return {'message': 'An error occurred while deleting a student from course'}, HTTPStatus.INTERNAL_SERVER_ERROR
            return {
...
class CourseStudentsListView(Resource):
    ...
    @courses_namespace.doc(
        description="""
            This endpoint is accessible to an admins and instructors. 
    ...
    def get(self, course_id):
        """
        Retrieve all registered students in a course
        """
        course = Course.get_by_id(course_id)
        get_student_in_course = StudentCourse.get_students_in_course_by(
            course.id)
...
class CoursesGradeListView(Resource):
    ...
    @courses_namespace.doc(
        description="""
            ...
            It allows students retrieve all registered courses grade
            """
    )
    @student_required()
    def get(self):
        ...
        for course in courses:
            grade_response = {}
            score_in_course = Score.query.filter_by(
                student_id=student.id, course_id=course.id).first()
